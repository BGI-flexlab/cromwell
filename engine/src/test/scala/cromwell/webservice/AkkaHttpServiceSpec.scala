...
class AkkaHttpServiceSpec extends AsyncFlatSpec with ScalatestRouteTest with Matchers {
  import AkkaHttpServiceSpec._

  val akkaHttpService = new MockApiService()
  val version = "v1"

  implicit def default(implicit system: ActorSystem) = RouteTestTimeout(5.seconds) // FIXME: hardcode this to match AkkaHttpService


    behavior of "REST API /status endpoint"
    it should "return 200 for get of a known workflow id" in {
      val workflowId = AkkaHttpServiceSpec.ExistingWorkflowId
    ...
    it should "return 404 for get of unknown workflow" in {
      val workflowId = AkkaHttpServiceSpec.UnrecognizedWorkflowId
      Get(s"/workflows/$version/$workflowId/status") ~>
        akkaHttpService.routes ~>
        check {
    ...
    it should "return 403 for abort of a workflow in a terminal state" in {
      Post(s"/workflows/$version/${AkkaHttpServiceSpec.AbortedWorkflowId}/abort") ~>
      akkaHttpService.routes ~>
      check {
        ...
        assertResult(
          s"""{
              |  "status": "error",
              |  "message": "Workflow ID '${AkkaHttpServiceSpec.AbortedWorkflowId}' is in terminal state 'Aborted' and cannot be aborted."
              |}""".stripMargin
        ) {
          responseAs[String]
        }
    ...
    it should "return 200 for abort of a known workflow id" in {
      Post(s"/workflows/$version/${AkkaHttpServiceSpec.ExistingWorkflowId}/abort") ~>
        akkaHttpService.routes ~>
        check {
          assertResult(
            s"""{"id":"${AkkaHttpServiceSpec.ExistingWorkflowId.toString}","status":"Aborted"}""") {
            responseAs[String]
          }
    ...
    it should "return 201 for a successful workflow submission " in {
      ...
      Post(s"/workflows/$version", formData) ~>
        ...
        check {
          assertResult(
            s"""{
                |  "id": "${AkkaHttpServiceSpec.ExistingWorkflowId.toString}",
                |  "status": "Submitted"
                |}""".stripMargin) {
    ...
    it should "return 200 with GET of outputs on successful execution of workflow" in {
      Get(s"/workflows/$version/${AkkaHttpServiceSpec.ExistingWorkflowId}/outputs") ~>
        akkaHttpService.routes ~>
        check {
          status should be(StatusCodes.OK)
    ...
    it should "return 404 with outputs on unknown workflow" in {
      Get(s"/workflows/$version/${AkkaHttpServiceSpec.UnrecognizedWorkflowId}/outputs") ~>
        akkaHttpService.routes ~>
      check {
        assertResult(StatusCodes.NotFound) {
          status
    ...
    it should "return 405 with POST of outputs on successful execution of workflow" in {
      Post(s"/workflows/$version/${AkkaHttpServiceSpec.UnrecognizedWorkflowId}/outputs") ~>
    ...
    it should "return 200 with paths to stdout/stderr/backend log" in {
      Get(s"/workflows/$version/${AkkaHttpServiceSpec.ExistingWorkflowId}/logs") ~>
        akkaHttpService.routes ~>
        check {
          status should be(StatusCodes.OK)
    ...
    it should "return 404 with logs on unknown workflow" in {
      Get(s"/workflows/$version/${AkkaHttpServiceSpec.UnrecognizedWorkflowId}/logs") ~>
        akkaHttpService.routes ~>
        check {
          assertResult(StatusCodes.NotFound) {
            status
    ...
    it should "return with full metadata from the metadata route" in {
      Get(s"/workflows/$version/${AkkaHttpServiceSpec.ExistingWorkflowId}/metadata") ~>
        akkaHttpService.routes ~>
        check {
          status should be(StatusCodes.OK)
    ...
    it should "return with included metadata from the metadata route" in {
      Get(s"/workflows/$version/${AkkaHttpServiceSpec.ExistingWorkflowId}/metadata?includeKey=testKey1&includeKey=testKey2a") ~>
       akkaHttpService.routes ~>
        check {
          status should be(StatusCodes.OK)
          val decoder: Decoder = Gzip
    ...
    it should "return with excluded metadata from the metadata route" in {
     Get(s"/workflows/$version/${AkkaHttpServiceSpec.ExistingWorkflowId}/metadata?excludeKey=testKey2b&excludeKey=testKey3") ~>
       akkaHttpService.routes ~>
        check {
          status should be(StatusCodes.OK)
          val decoder: Decoder = Gzip
    ...
    it should "return an error when included and excluded metadata requested from the metadata route" in {
      Get(s"/workflows/$version/${AkkaHttpServiceSpec.ExistingWorkflowId}/metadata?includeKey=testKey1&excludeKey=testKey2") ~>
        akkaHttpService.routes ~>
        check {
          assertResult(StatusCodes.BadRequest) {
            status
    ...
    it should "return 200 with an HTML document for the timings route" in {
      Get(s"/workflows/$version/${AkkaHttpServiceSpec.ExistingWorkflowId}/timing") ~>
        akkaHttpService.routes ~>
        check {
    ...
    it should "return good results for a good query" in {
      Get(s"/workflows/$version/query?status=Succeeded&id=${AkkaHttpServiceSpec.ExistingWorkflowId}") ~>
        akkaHttpService.routes ~>
        check {
          status should be(StatusCodes.OK)
          contentType should be(ContentTypes.`application/json`)
          val results = responseAs[JsObject].fields("results").convertTo[Seq[JsObject]]
          results.head.fields("id") should be(JsString(AkkaHttpServiceSpec.ExistingWorkflowId.toString))
...
object AkkaHttpServiceSpec {
  ...
  class MockApiService()(implicit val system: ActorSystem) extends AkkaHttpService {
